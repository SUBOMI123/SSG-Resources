  ...
  return (
    <>
      <AdminPageHeader
        ...
        description="Use this page to check customer details, items ordered, payment history, and anything you need to update."
      ...
      <AdminSectionTabs
        tabs={[
          ...
          { href: `/admin/orders/${order.id}`, label: "Details", active: true },
        ]}
      />
      ...
      <section className="admin-stat-grid">
        ...
        <AdminMetricCard label="Payment" value={simplePaymentStatus(order.payment_status)} />
        <AdminMetricCard label="Stage" value={simpleOrderStage(order.fulfillment_status)} tone="accent" />
      </section>
        ...
        <article className="admin-panel admin-detail-card">
          <div className="admin-panel-head">
            <div>
              ...
              <h2 className="admin-section-title">Order information</h2>
            </div>
            <div className="admin-inline-actions">
              <Link className="secondary-pill" href={`/documents/invoice/${order.reference}`}>
                Invoice
              </Link>
              <Link className="secondary-pill" href={`/documents/waybill/${order.reference}`}>
                Waybill
              </Link>
              <Link className="secondary-pill" href={`/orders/${order.reference}`}>
                Tracking
              </Link>
            </div>
          </div>
          <dl className="admin-detail-list">
            ...
            <div>
              <dt>Handled by</dt>
              <dd>{order.assigned_to ?? "Not assigned"}</dd>
            </div>
            <div>
              <dt>Added</dt>
              ...
              {order.payments.length === 0 ? (
                <div className="admin-list-row static">
                  <div>
                    ...
                    <span>This order is still waiting for payment.</span>
                  </div>
                </div>
              ) : (
        ...
        <article className="admin-panel admin-detail-card">
          <div className="admin-panel-head">
            <div>
              ...
              <h2 className="admin-section-title">Update this order</h2>
            </div>
          </div>
          <OrderDetailActions
            order={{
